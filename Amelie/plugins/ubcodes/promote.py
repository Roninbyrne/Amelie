    ...
    async def promote_user_common(client: Client, message: Message, full: bool = False):
        ...
        try:
            ...
            if bot_member.status != ChatMemberStatus.ADMINISTRATOR:
                await message.reply_text("❌ I'm not an admin in this group.")
                return
            if not bot_member.privileges.can_promote_members:
                await message.reply_text("❌ I don't have rights to promote users.")
                return
        except Exception as e:
            await message.reply_text(f"⚠️ Error retrieving my status: {e}")
        ...
        if target_user_id == bot_user.id:
            await message.reply_text("😐 Promoting myself? I'm flattered.")
            return

        if message.from_user.id == target_user_id:
            await message.reply_text("❌ You can't promote yourself.")
            return
